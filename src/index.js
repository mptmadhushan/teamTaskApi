...
const getToken = (user) => jwt.sign({ id: user._id }, 'pmurvk4NVK7NCqEtESVRrtwGhNkQqa3MBuj9tFjSRWS34SEbWevdkXPPSvdCVDSs6bFQg59cJUweRNKEJXtgD5pVdrDhhDFMr7g', { expiresIn: '30 days' });

const getUserFromToken = async (token, db) => {
  if (!token) { return null }

  const tokenData = jwt.verify(token, 'pmurvk4NVK7NCqEtESVRrtwGhNkQqa3MBuj9tFjSRWS34SEbWevdkXPPSvdCVDSs6bFQg59cJUweRNKEJXtgD5pVdrDhhDFMr7g');
  if (!tokenData?.id) {
    return null;
  }
...
const start = async () => {
  const uri = "mongodb+srv://root:Z1xnv0qQRLiKw5xs@cluster0.vzwtfsc.mongodb.net/?retryWrites=true&w=majority";
